...
describe("04 - spend", () => {
    ...
    before(async () => {
        ...
        providerTokenAccount = await (0, spl_token_1.createAccount)(provider.connection, owner.payer, usdcMint, providerWallet.publicKey);
        console.log("\n  --- Addresses ---");
    ...
    it("Agent spends 5 USDC to a provider wallet", async () => {
        ...
        const agentTokenBefore = (await (0, spl_token_1.getAccount)(provider.connection, agentTokenAccount)).amount;
        const providerBalBefore = (await (0, spl_token_1.getAccount)(provider.connection, providerTokenAccount)).amount;
        ...
        const agentTokenAfter = (await (0, spl_token_1.getAccount)(provider.connection, agentTokenAccount)).amount;
        const providerBalAfter = (await (0, spl_token_1.getAccount)(provider.connection, providerTokenAccount)).amount;
        // ── Assertions ────────────────────────────────────────────
        // 1. agent.balance must decrease by spend amount
        chai_1.assert.equal(agentAfter.balance.toString(), (BigInt(agentBefore.balance.toString()) - BigInt(spendAmount.toString())).toString(), "agent.balance should decrease by spend amount");
        // 2. agent.totalSpent must increase by spend amount
        chai_1.assert.equal(agentAfter.totalSpent.toString(), spendAmount.toString(), "agent.totalSpent should accumulate the spend");
        // 3. agent.spentThisWindow must track the window spend
        chai_1.assert.equal(agentAfter.spentThisWindow.toString(), spendAmount.toString(), "agent.spentThisWindow should record 5 USDC");
        // 4. Agent token account must be debited
        chai_1.assert.equal((agentTokenBefore - agentTokenAfter).toString(), spendAmount.toString(), "agent token account should be debited by spend amount");
        // 5. Provider token account must be credited
        chai_1.assert.equal((providerBalAfter - providerBalBefore).toString(), spendAmount.toString(), "provider token account should receive the spend amount");
