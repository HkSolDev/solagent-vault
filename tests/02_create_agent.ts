...
describe("02 - create_agent", () => {
  ...
  it("Creates an agent PDA and funds the agent keypair with gas SOL", async () => {
    ...
    assert.equal(agent.vault.toString(), vaultStatePda.toString(),
      "agent.vault should point to the correct vault PDA");
    ...
    assert.equal(agent.owner.toString(), owner.publicKey.toString(),
      "agent.owner should match the transaction signer");
    ...
    assert.equal(agent.maxPerCall.toString(), maxPerCall.toString(),
      "agent.maxPerCall should match the parameter");
    assert.equal(agent.maxPerMinute.toString(), maxPerMinute.toString(),
      "agent.maxPerMinute should match the parameter");

    // 7. Rate-limit counters must start clean
    assert.equal(agent.spentThisWindow.toString(), "0",
      "agent.spentThisWindow should start at 0");
    assert.equal(agent.totalSpent.toString(), "0",
      "agent.totalSpent should start at 0");
    ...
    assert.equal(agentSignerBalance, 200_000_000,
      "agentSigner should have received 0.2 SOL gas allocation");
