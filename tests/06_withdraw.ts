...
describe("06 - withdraw", () => {
  ...
  it("Withdraws 20 USDC from agent back to owner (partial withdraw)", async () => {
    ...
    const agentBalAfter = (await getAccount(provider.connection, agentTokenAccount)).amount;
    ...
    assert.equal(
      ...
      (BigInt(agentBefore.balance.toString()) - BigInt(withdrawAmount.toString())).toString(),
      "agent.balance should decrease by withdraw amount"
    );

    // 2. Agent token account must be debited
    assert.equal(
      (agentBalBefore - agentBalAfter).toString(),
      withdrawAmount.toString(),
      "agent token account should be debited by withdraw amount"
    );
    ...
    assert.equal(
      ...
      withdrawAmount.toString(),
      "owner token account should receive the withdraw amount"
    );
    ...
    assert.equal(
      agentAfter.balance.toString(),
      "30000000",
      "agent should have 30 USDC remaining"
    );
  ...
  it("Reverts when withdrawing more than agent balance (InsufficientBalance)", async () => {
    ...
    try {
      await program.methods
        ...
        .rpc();
      assert.fail("Should have reverted with InsufficientBalance");
    } catch (err: any) {
